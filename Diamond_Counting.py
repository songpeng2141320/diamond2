...
def draw_count(tmp,s1,s2,s,flag=False):
    ...
    sns.barplot(x=tmp.index,y=tmp.values/tmp.values.sum(),palette=sns.hls_palette(n_colors=tmp.shape[0], h=0.01, l=0.6, s=0.65))
...
def main():
    st.write("### 2021年-2024年交易数据")
    df = get_data(FILENAME)
    df["range"] = 1

    with st.sidebar:
        st.write("### Select:")
        s1 = st.selectbox(label="年份",options=["2024","2023","2022","2021","All"])
        s2 = st.multiselect(label="季度",options=["Q1","Q2","Q3","Q4"],default="Q1")
